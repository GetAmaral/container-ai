...
// ═══════════════════════════════════════════════════
// NOVO: getValidAccessToken (extraido da function principal)
// Garante que o token esta valido antes de chamar a Google API
// ═══════════════════════════════════════════════════
...
serve(async (req) => {
  ...
  try {
    ...
    // sync = confirmacao inicial do webhook
    ...
    if (resourceState === 'exists') {
      ...
      // Buscar usuario pelo webhook_id (isolamento por user)
...
async function performIncrementalSync(userId: string) {
  ...
  try {
    // CORRECAO: obter token valido (renova se expirado)
    ...
    do {
      ...
      if (nextPageToken) {
        ...
        // Fallback: ultimos 30 dias e proximos 90 dias
      ...
      if (!response.ok) {
        if (response.status === 410) {
          // Sync token expirado, limpar e recomecar
...
// ═══════════════════════════════════════════════════
// Processar mudanca de evento individual
// (sem alteracao — logica original preservada)
// ═══════════════════════════════════════════════════
async function processEventChange(userId: string, gEvent: any) {
  try {
    ...
    if (!gEvent.start?.dateTime || !gEvent.end?.dateTime) {
      return; // Pular eventos de dia inteiro (sera corrigido na Fase 5)
    }

    // Verificar se evento ja existe (filtrado por user_id)
