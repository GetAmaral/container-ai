...
function parseState(state) {
  try {
    // Tenta parsear como JSON (novo formato: {userId, origin})
    ...
    // Fallback para formato antigo (apenas userId)
    return {
      userId: state,
...
function createState(userId, origin) {
...
serve(async (req)=>{
  ...
  // A) Início do fluxo: sem code e sem error -> redireciona pro OAuth do Google
  if (req.method === 'GET' && !code && !error) {
    ...
    // Cria state com userId e origin para usar no callback
  ...
  // B) Callback: se houver erro do Google - redireciona para página do frontend
  if (req.method === 'GET' && error) {
    console.error('OAuth error:', error);
    const { origin } = state ? parseState(state) : {
      origin: PRODUCTION_URL
    };
  ...
  // JSON API para as outras ações - Requires authentication
  try {
    ...
    let finalUserId;
    // Se o token for a service role key, confiamos no userId do corpo
    if (token === supabaseServiceRoleKey) {
      if (!userId) {
        return new Response(JSON.stringify({
          error: 'Missing userId for service role request'
        }), {
          status: 400,
          headers: {
            ...corsHeaders,
      ...
      // Caso contrário, validamos o JWT do usuário
      ...
      // Se for um usuário autenticado, ele SÓ pode agir sobre seu próprio ID
      // Se um userId foi passado no body, ele DEVE ser igual ao do token
...
async function handleAuth(userId) {
...
async function handleCallback(userId, code) {
  try {
    ...
    // Armazena/atualiza a conexão do usuário com criptografia
    ...
    // Dispara sincronização inicial + registro de webhook em background
...
async function getValidAccessToken(userId) {
  try {
    const { data: tokens, error } = await supabase.rpc('secure_get_google_tokens', {
      p_user_id: userId
    }).single();
    if (error) {
    ...
    if (tokens.expires_at && new Date() >= new Date(tokens.expires_at)) {
      ...
      if (refreshedToken) {
        ...
        await supabase.rpc('reset_failed_token_access', {
          p_user_id: userId
        });
      }
...
async function refreshAccessToken(userId, refreshToken) {
  try {
    ...
    // Use encrypted function to store the new access token
...
async function handleCreateEvent(userId, eventData) {
  try {
...
async function handleUpdateEvent(userId, eventId, eventData) {
  try {
...
async function handleDisconnect(userId) {
  try {
    ...
    // 1. Cancelar webhook (best effort)
    try {
      await cancelGoogleWebhook(userId);
    } catch (error) {
      console.error('Error canceling webhook:', error);
    }
    // 2. Revogar token do Google (best effort)
    try {
      const { data: tokens } = await supabase.rpc('secure_get_google_tokens', {
        p_user_id: userId
      }).single();
    ...
    // 3. Remover eventos do Google Calendar (preserva eventos locais)
    const { data: deletedCount } = await supabase.rpc('remove_google_calendar_events', {
      p_user_id: userId
    }).single();
...
async function handleSyncFromGoogle(userId) {
  try {
    console.log(`Manual sync requested for user: ${userId}`);
    // Rate limiting: verificar última sincronização
    const { data: connection } = await supabase.from('google_calendar_connections').select('last_sync_at').eq('user_id', userId).single();
...
async function performInitialSync(userId) {
  try {
    ...
    // Buscar eventos dos últimos 1 mês e próximos 6 meses
    ...
    url.searchParams.set('maxResults', '500'); // Limitar para performance
    url.searchParams.set('singleEvents', 'true'); // Expandir eventos recorrentes
    ...
    const nextSyncToken = data.nextSyncToken; // Capture the sync token
    console.log(`📥 Found ${googleEvents.length} events in Google Calendar`);
    // Buscar eventos existentes do usuário (otimizado)
    const { data: existingEvents, error: fetchError } = await supabase.from('calendar').select('session_event_id_google, event_name, start_event').eq('user_id', userId).not('session_event_id_google', 'is', null);
    if (fetchError) {
    ...
    // Inserir em batch (máximo 100 por vez para segurança)
    if (eventsToInsert.length > 0) {
      ...
      for(let i = 0; i < eventsToInsert.length; i += 100){
        ...
        const { error: insertError, count } = await supabase.from('calendar').insert(batch);
        if (insertError) {
    ...
    // Atualizar timestamp de última sincronização e o sync_token
    const updateData = {
      last_sync_at: new Date().toISOString()
    };
    if (nextSyncToken) {
    ...
    await supabase.from('google_calendar_connections').update(updateData).eq('user_id', userId);
...
// =============================================
// FUNÇÕES DE WEBHOOK E SINCRONIZAÇÃO BIDIRECIONAL
// =============================================
async function setupGoogleWebhook(userId) {
  ...
  // Não falhar a conexão se webhook falhar
  }
}
async function cancelGoogleWebhook(userId) {
  try {
    console.log(`🔕 Canceling webhook for user: ${userId}`);
    const { data: connection } = await supabase.from('google_calendar_connections').select('webhook_id, webhook_resource_id').eq('user_id', userId).single();
...
async function handleCronSync(userId, renewWebhook = false) {
  try {
...
async function performIncrementalSync(userId) {
  ...
  try {
    ...
    const { data: connection } = await supabase.from('google_calendar_connections').select('sync_token').eq('user_id', userId).single();
...
async function processEventChange(userId, gEvent) {
  try {
    ...
    if (gEvent.status === 'cancelled') {
      ...
      await supabase.from('calendar').delete().eq('user_id', userId).eq('session_event_id_google', googleEventId);
      return;
    }
    // Validar campos obrigatórios
    if (!gEvent.start?.dateTime || !gEvent.end?.dateTime) {
    ...
    const { data: existing } = await supabase.from('calendar').select('id, event_name, start_event, end_event, desc_event').eq('user_id', userId).eq('session_event_id_google', googleEventId).maybeSingle();
    const eventData = {
      event_name: (gEvent.summary || 'Sem título').substring(0, 255),
      desc_event: (gEvent.description || '').substring(0, 5000),
      start_event: gEvent.start.dateTime,
    ...
    if (existing) {
      ...
      const hasChanged = existing.event_name !== eventData.event_name || existing.start_event !== eventData.start_event || existing.end_event !== eventData.end_event || (existing.desc_event || '') !== eventData.desc_event;
      if (hasChanged) {
