...
// Mudancas: batch 5 (era 10), delay 2s entre batches, rate limit 10min
// =============================================================================
...
serve(async (req) => {
  ...
  try {
    ...
    // Batch de 5 (era 10) com delay de 2s entre batches
    const batchSize = 5;
    for (let i = 0; i < connections.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (conn) => {
          try {
            // Rate limit: pular se sincronizou ha menos de 10 minutos
      ...
      // Delay de 2s entre batches (exceto o ultimo)
